...
Two demonstrations exist, and they are not interchangeable:

- `demo/` **in this repository** (generated by `make demo`) is a *structure
demonstration only*: it shows what a generated publisher artifact looks like.
Do not use it for anything durable — its keystore (`keryx/.demo-keys`,
gitignored) may be lost or committed by accident, and without the master key
the artifact can never be re-signed. Its keys are disposable; regenerate
freely.
- [`../keryx-demo`](https://keryx-demo.github.io/) is the **live
demonstration site** for the tooling. Its keys live in
`../keryx-demo-keys`, outside every repository, and are never committed;
see [`keryx-demo/README.md`](../keryx-demo/README.md).
...
- `demo/` — a generated demonstration publisher artifact (full TUF repo +
  ...
  committed — regenerate it with `make demo` (below). Structure demonstration
  only, not for durable use: its keys are disposable and may be lost or public;
  the live site and its keys are described in the Demo section above
