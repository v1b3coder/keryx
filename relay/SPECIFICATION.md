...
- `nonce` is generated by the client and echoed by the relay. A client accepts
  a test only while it is waiting for that nonce (one pending nonce per
  install, cleared on use or expiry). A payload without the pending nonce MUST
  NOT be reported as a successful test.
...
| `POST /v1/registrations/{id}/test` | `{ "nonce": "<43 chars>" }` | Deliver one self-test payload (§4.3) to this registration's endpoint. Management token required; `202` acknowledges the provider attempt, not device receipt. |
| `POST /v1/fcm/test` | `{ "token": "<FCM registration token>", "nonce": "<43 chars>" }` | Deliver one targeted FCM self-test (§4.3) to that device token. `202`; the token is transient and MUST NOT be stored. |
...
- The endpoint test requires the registration's management token and delivers
  the §4.3 payload through the ordinary endpoint path (RFC 8291, VAPID, TTL,
  outbound policy). `202` acknowledges the provider attempt, not device
  receipt.
- The FCM test targets a single `message.token` (never a topic) with the
  §4.3 payload as a data-only message (`message.data.test`, one JSON
  string). The token is used for that one send and MUST NOT be stored,
  logged, traced, or retained in analytics: the topic leg stays
  registry-free and anonymous (§9). It is unauthenticated — the FCM token is
  itself an unguessable capability — and admitted only within its own
  per-IP and global budget.
...
- The client reports "delivered" only when its own handler observes the
  ...
  claim it.
- Errors: `404` unknown, expired, or GC'd registration (an unknown FCM token
  is counted as a provider failure, not a registration lookup), `401` invalid
...
- **Publish:** `POST https://fcm.googleapis.com/v1/projects/<project>/messages:send`
  ...
  omitting `t`.
- **Self-test:** `message.token` (a single device, never a topic) with the
  §4.3 payload under `message.data.test` — a one-shot, user-triggered send
  that never stores the token (§5.3.1).
...
- **Self-test:** the §4.3 payload is unauthenticated but nonce-bound and is
  ...
  notice, within the relay-spam bound above. The FCM test token is transient —
  never persisted, logged, or traced.
...
- A self-test is never a wake-up: it advances no sequence, grants no content
  ...
  client's matching pending nonce counts as delivered, and the FCM test token
