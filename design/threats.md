...
| Attacker who stole a *channel* key | Scoped to its own channel: can publish, unpublish and re-pin that channel's content (availability + which content is shown); in an authored channel (the default) **cannot forge items** (item signatures are author-signed) — only re-pin/withhold/unpublish; in simple mode it is the content authority; revocation = signed metadata update |
...
| Push provider linkage | APNs/FCM see the device↔company mapping (token↔topic subscriptions); ntfy's anonymous topics remove identity linkage, but topic relationships remain visible to the relay operator ([`why.md` §4.10](why.md)) |
